...
I2cDevice::I2cDevice(std::uint8_t address, const std::string& devicePath)
    : address_(address) {
...
void I2cDevice::writeRegister16(std::uint8_t reg, std::uint16_t value) {
    ...
    if (::write(fd_, data, sizeof(data)) != static_cast<ssize_t>(sizeof(data)))
        throw systemError("I2C write failed");
}
...
void I2cDevice::writeRegister8(std::uint8_t reg, std::uint8_t value) const {
    ...
    if (::write(fd_, data, sizeof(data)) != static_cast<ssize_t>(sizeof(data)))
        throw systemError("I2C register write failed");
}
...
void I2cDevice::readRegisters(std::uint8_t startReg, std::uint8_t* buffer, std::size_t length) const {
    if (buffer == nullptr || length == 0 || length > std::numeric_limits<__u16>::max())
        throw std::invalid_argument("Invalid I2C read buffer");
    ...
    if (::ioctl(fd_, I2C_RDWR, &transaction) < 0)
        throw systemError("I2C combined read failed");
}
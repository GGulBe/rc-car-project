#!/usr/bin/env python3
"""Visual OV5647 + INT8 person detector test for Raspberry Pi 4.
...
def parse_args():
    ...
    parser.add_argument("--max-temperature", type=float, default=78.0)
    parser.add_argument("--no-preview", action="store_true")
    parser.add_argument("--output", type=Path, default=Path("pi_int8_camera_results"))
...
def draw(frame, boxes, threshold, inference_ms, total_ms, temp, scenario):
    ...
    cv2.imshow("RC Person Detector INT8 - numeric results only", display)
...
def main():
    ...
    model_path = base / "person_detector_int8.onnx"
    if not model_path.is_file():
        raise FileNotFoundError(model_path)

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_dir = (base / args.output / stamp).resolve()
    result_dir.mkdir(parents=True, exist_ok=True)
    frame_path = result_dir / "frame_metrics.csv"
    detection_path = result_dir / "detections.csv"

    print("=" * 72)
    print("OV5647 + INT8 PERSON DETECTOR VISUAL TEST")
    ...
    try:
        with frame_path.open("w", newline="", encoding="utf-8") as frame_file, \
            ...
            while time.perf_counter() - start < args.duration:
                ...
                boxes, postprocess_ms = decode(output[0], threshold, args.nms)
                ...
                scale_x = frame.shape[1] / IMAGE_WIDTH
                scale_y = frame.shape[0] / IMAGE_HEIGHT
                for detection_index, box in enumerate(boxes, start=1):
                    ...
                    detection_writer.writerow({
                        ...
                        "camera_xmin": x1 * scale_x, "camera_ymin": y1 * scale_y,
                        "camera_xmax": x2 * scale_x, "camera_ymax": y2 * scale_y,
                        "center_x": center_x * scale_x, "center_y": center_y * scale_y,
                        "bottom_center_x": center_x * scale_x,
                        "bottom_center_y": y2 * scale_y,
                        "normalized_center_x": center_x / IMAGE_WIDTH,
                        "normalized_center_y": center_y / IMAGE_HEIGHT,
                        "normalized_bottom_center_x": center_x / IMAGE_WIDTH,
                        "normalized_bottom_center_y": y2 / IMAGE_HEIGHT,
